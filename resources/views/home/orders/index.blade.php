...
@section('title', 'Pesanan Saya - FASHION SAAZZ')
  ...
  <div class="orders-section">
    <div class="container">
      <div class="orders-header mb-4">
        ...
        <p class="text-muted">Lacak dan kelola pesanan Anda</p>
      </div>
          ...
          <div class="search-box mb-4">
            <form method="GET" action="{{ route('orders.index') }}">
              ...
              <div class="input-group">
                <input type="text" class="form-control" name="search" placeholder="Cari pesanan..." value="{{ $search }}">
              ...
              @foreach($orders as $order)
                <div class="order-item-list">
                      ...
                      <div class="item-preview">
                        ...
                        <div class="item-info-tiny">
                          ...
                          @if($order->items->count() > 1)
                            <small class="text-muted">+{{ $order->items->count() - 1 }} lainnya</small>
                          @endif
                        </div>
                      </div>
                    @endforeach
                  ...
                  <div class="order-actions-list">
                    <a href="{{ route('orders.show', $order->order_number) }}" class="action-btn" title="Lihat Detail">
                      <i class="fa fa-eye"></i>
                    </a>
                    @if($order->status === 'pending' && $order->paymentTransaction && $order->paymentTransaction->status === 'pending')
                      <a href="{{ route('checkout.payment', $order->order_number) }}" class="action-btn action-btn-success"
                        title="Bayar Sekarang">
                        <i class="fa fa-credit-card"></i>
                      </a>
                    @endif
                    @if($order->status === 'shipped')
                      <button type="button" class="action-btn action-btn-success" title="Konfirmasi Diterima"
      ...
      <div class="col-lg-4">
        <h6 class="mb-3 text-muted">Mungkin Anda suka</h6>
        <div class="recommended-list">
          @foreach($recommendedProducts as $product)
            <div class="product-item">
              ...
              <a href="{{ route('product.detail', $product->id) }}" class="product-view-btn" title="Lihat Produk">
                <i class="fa fa-eye"></i>
              </a>
            </div>
          @endforeach
